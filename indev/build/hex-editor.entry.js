import { f as forceUpdate, r as registerInstance, e as createEvent, h } from './index-fca4abca.js';

var EditingMode;
(function (EditingMode) {
  EditingMode[EditingMode["ASCII"] = 0] = "ASCII";
  EditingMode[EditingMode["BYTE"] = 1] = "BYTE";
  EditingMode[EditingMode["BIT"] = 2] = "BIT";
})(EditingMode || (EditingMode = {}));
var SearchType;
(function (SearchType) {
  SearchType[SearchType["ASCII"] = 0] = "ASCII";
  SearchType[SearchType["BYTE"] = 1] = "BYTE";
  SearchType[SearchType["INTEGER"] = 2] = "INTEGER";
  SearchType[SearchType["FLOAT"] = 3] = "FLOAT";
})(SearchType || (SearchType = {}));
var NonLetter;
(function (NonLetter) {
  NonLetter["DOT"] = "\u2022";
  NonLetter["CROSS"] = "\u2573";
  NonLetter["FULL_BLOCK"] = "\u2588";
  NonLetter["LIGHT_SHADE"] = "\u2591";
  NonLetter["MEDIUM_SHADE"] = "\u2592";
  NonLetter["DARK_SHADE"] = "\u2593";
  NonLetter["DASH"] = "-";
  NonLetter["PLUS"] = "+";
})(NonLetter || (NonLetter = {}));
...
const HexEditor = class {
  constructor(hostRef) {
    ...
    this.wheel = (evt) => {
      ...
      if (scaledVelocity === -0)
        scaledVelocity -= 1;
      if (Math.abs(evt.deltaY) > 70)
        scaledVelocity *= 0.10;
      if (evt.ctrlKey && evt.shiftKey)
        scaledVelocity = Math.sign(scaledVelocity) * this.maxLines;
      else if (evt.shiftKey)
        scaledVelocity = Math.sign(scaledVelocity);
      else if (evt.ctrlKey)
        scaledVelocity *= 20;
    ...
    this.searchType = SearchType.ASCII;
    ...
    this.nonDisplayCharacter = NonLetter.DOT;
    this.nonDisplayOpacity = 0.45;
    this.chunks = [];
  ...
  */
  ...
  buildHexView() {
    ...
    for (const [lineNum, line] of lines.entries()) {
      ...
      const charLines = [];
      const hexLines = [];
      let ascii = this.nonDisplayCharacter;
    ...
    const buildRegion = (region, depth = 0, index) => {
      ...
      const genPolygon = (width, height) => (h("polygon", { onMouseMove: (evt) => {
          ...
          var _a, _b, _c;
          if (document.getElementById('tooltip').getAttribute('active') === "frozen") {
            if (((_c = (_b = (_a = document.getSelection().anchorNode) === null || _a === void 0 ? void 0 : _a.parentElement) === null || _b === void 0 ? void 0 : _b.parentElement) === null || _c === void 0 ? void 0 : _c.tagName) === "HEX-TOOLTIP") {
  ...
  showHex() {
    ...
    return (h("div", { class: "hex", onMouseEnter: (evt) => this._toggleScrollListener(evt), onMouseLeave: (evt) => this._toggleScrollListener(evt), onMouseDown: (evt) => this.beginSelection(evt), onMouseUp: (evt) => this.endSelection(evt), tabindex: "0", onKeyDown: (evt) => this.edit(evt) }, h("div", { id: "MEASURE", class: "hex", style: { position: 'absolute', visibility: 'hidden', padding: '0 5px' } }, "AB"), h("div", { class: "lineLabels" }, lineLabels), this.displayBin ?
      ...
      h("div", { class: "find" }, "search:", h("input", { type: "text", onChange: (evt) => this.searchInput = evt.target.value }), h("select", { onChange: (evt) => this.searchType = evt.target.value }, h("option", { value: SearchType.ASCII }, "ASCII string"), h("option", { value: SearchType.BYTE }, "bytes"), h("option", { value: SearchType.INTEGER }, "integer"), h("option", { value: SearchType.FLOAT }, "float")), ([SearchType.INTEGER, SearchType.FLOAT].includes(this.searchType)) ? [
  ...
  beginSelection(evt) {
    ...
    if (parentClassName.includes('charLine'))
      this.editingMode = EditingMode.ASCII;
    else if (parentClassName.includes('hexLine'))
      this.editingMode = EditingMode.BYTE;
    else if (parentClassName.includes('binGroup'))
      this.editingMode = EditingMode.BIT;
    else
      return;
  ...
  _toggleScrollListener(evt) {
    if (evt.type === "mouseenter") {
      evt.target.addEventListener("wheel", this.wheel, { passive: false });
    }
    else {
      evt.target.removeEventListener("wheel", this.wheel, false);
    }
  }
};
HexEditor.style = hexEditorCss;

export { HexEditor as hex_editor };

//# sourceMappingURL=hex-editor.entry.js.map